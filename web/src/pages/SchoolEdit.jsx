...
const SchoolEdit = () => {
  ...
  useSelect2([lookupLoading, lists.provinces.length, lists.khet.length, lists.schoolSize.length, loading]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const { data, error } = await supabase.from('tbl_school').select('*').eq('school_id', schoolIdParam).maybeSingle();
        if (error) throw error;
        if (!data) {
          Swal.fire('Error', 'ไม่พบข้อมูลโรงเรียน', 'error');
          navigate('/school');
          return;
        }
        if (mounted) {
          setForm({
            ...
            school_code6: data.school_code6 || '',
            school_code8: data.school_code8 || '',
            school_id_onet: data.school_id_onet || '',
            school_name: data.school_name || '',
            school_add: data.school_add || '',
            school_trok: data.school_trok || '',
            school_soi: data.school_soi || '',
            ...
            school_telephone1: data.school_telephone1 || '',
            school_telephone2: data.school_telephone2 || '',
    ...
    if (schoolIdParam) load();
    return () => { mounted = false; };
  }, [schoolIdParam, navigate]);
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          school_code6: form.school_code6,
          school_code8: form.school_code8,
          school_id_onet: form.school_id_onet,
          school_add: form.school_add,
          school_trok: form.school_trok,
          school_soi: form.school_soi,
          school_street: form.school_street,
          district_name: form.district_name,
          subdistrict_name: form.subdistrict_name,
          school_province: form.school_province,
          school_postcode: form.school_postcode,
          school_telephone1: form.school_telephone1,
          school_telephone2: form.school_telephone2,
          school_fax: form.school_fax,
  ...
  const readOnlySchoolId = Boolean(form.school_id);
  const readOnlyCode6 = Boolean(form.school_code6);
  const readOnlyCode8 = Boolean(form.school_code8);
  const readOnlyOnet = Boolean(form.school_id_onet);
            ...
            <form onSubmit={handleSubmit}>
              <div className="row">
                <div className="mb-3 col-6">
                  <label htmlFor="school_id">รหัส MOECODE (รหัสกระทรวง):</label>
                  <input type="text" className="form-control" id="school_id" name="school_id" placeholder="รหัส MOECODE" maxLength="10" minLength="10" value={form.school_id} onChange={handleChange} readOnly={readOnlySchoolId} />
                </div>
                <div className="mb-3 col-6">
                  <label htmlFor="school_code6">รหัสโรงเรียน 6 หลัก (รหัส Obec):</label>
                  <input type="text" className="form-control" id="school_code6" name="school_code6" placeholder="รหัสโรงเรียน 6 หลัก (Obec Code ของ สพฐ.)" maxLength="6" minLength="6" value={form.school_code6} onChange={handleChange} readOnly={readOnlyCode6} />
                </div>
              </div>
              <div className="row">
                <div className="mb-3 col-6">
                  <label htmlFor="school_code8">รหัสโรงเรียน 8 หลัก (รหัส SMIS):</label>
                  <input type="text" className="form-control" id="school_code8" name="school_code8" placeholder="รหัสโรงเรียน 8 หลัก (รหัส SMIS)" maxLength="8" minLength="8" value={form.school_code8} onChange={handleChange} readOnly={readOnlyCode8} />
                </div>
                <div className="mb-3 col-6">
                  <label htmlFor="school_id_onet">รหัสโรงเรียน 10 หลัก ONET:</label>
                  <input type="text" className="form-control" id="school_id_onet" name="school_id_onet" placeholder="รหัสโรงเรียน 10 หลัก ห้ามซ้ำกับที่มีอยู่" maxLength="10" minLength="10" value={form.school_id_onet} onChange={handleChange} readOnly={readOnlyOnet} />
                </div>
              </div>
              <div className="mb-3">
