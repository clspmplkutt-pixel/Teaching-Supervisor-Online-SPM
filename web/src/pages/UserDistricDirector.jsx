  ...
  const handleRemove = (peopleId) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        navigate(`/user_remove?people_id=${peopleId}&from=userDistricDirector`);
      }
    });
  };
      ...
      <div className="col-sm-12 col-md-12 col-lg-12 col-xl-12">
        <div className="card card-success">
          <div className="card-header">
            <h3 className="card-title"><i className="fa-solid fa-school-circle-check"></i> ผู้เข้าใช้งานระบบ ผู้อำนวยการเขตพื้นที่การศึกษา/รองผู้อำนวยการเขตพื้นที่การศึกษา</h3>
          </div>
          <div className="card-body">
            <div className="col-sm-12">
              <span className="float-sm-right">
                <Link className="btn btn-outline-info" to="/register"><i className="fa-solid fa-plus"></i> เพิ่มผู้ใช้งาน</Link>
              </span>
            </div>
                    ...
                    <tr key={row.id}>
                      ...
                      <td>
                        <div className="btn-group">
                          {row.people_id ? (
                            <Link to={`/dd_edit?people_id=${row.people_id}`} className="btn btn-warning btn-sm">
                              <i className="fa-solid fa-pen-to-square"></i> แก้ไข
                            </Link>
                          ) : (
                            <Link to={`/dd_edit?id=${row.id}`} className="btn btn-warning btn-sm">
                              <i className="fa-solid fa-pen-to-square"></i> แก้ไข
                            </Link>
                          )}
                          ...
                          <Link to={`/resetPwd?id=${row.id}&from_module=userDistricDirector`} className="btn btn-info btn-sm">
...
export default UserDistricDirector;
